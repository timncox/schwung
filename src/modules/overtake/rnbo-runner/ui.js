...
globalThis.tick = function() {
    ...
    if (phase === 3) {
        ...
        host_system_cmd('sh -c "export HOME=/data/UserData LD_LIBRARY_PATH=' + RNBO_DIR + '/lib:$LD_LIBRARY_PATH && nohup taskset 0x7 ' + CONTROL_BIN + ' -s ' + SHADOW_CONFIG + ' > /data/UserData/schwung/rnbo-runner.log 2>&1 &"');
    ...
    // SPI driver runs at FIFO 90 on core 3 — RNBO threads on core 3
    // cause cache/memory contention that spikes ioctl latency.
    // No chrt needed — shadow_ui now launches at SCHED_OTHER.
    if (phase === 50) {
        host_system_cmd('sh -c "for p in $(pgrep -f jackd) $(pgrep rnbomovecontrol) $(pgrep rnbooscquery); do taskset -p 0x7 $p 2>/dev/null; done &"');
    }
    ...
    if (phase > 440 && phase % 220 === 0) {
        host_system_cmd('sh -c "[ ! -f /data/UserData/schwung/jack_midi_connect.pid ] && (echo $$ > /data/UserData/schwung/jack_midi_connect.pid && LD_LIBRARY_PATH=' + RNBO_DIR + '/lib /data/UserData/schwung/bin/jack_midi_connect; rm -f /data/UserData/schwung/jack_midi_connect.pid) &"');
    }
